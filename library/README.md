...
│   ├── constants_library.json         # Main library: 5,303 cells × 21 classes × 11 observables
│   ├── constants_chi1.json            # χ₁-dependent Cβ angle corrections (2,987 entries)
...
**Not currently in this release:** an earlier draft of this table also listed `omega_deg_eq`, `*_std`, `*_k`, and `*_coupling` fields. None of these are present in the shipped `constants_library.json` (checked: 0 of 5,845 cells contain any of them). `lib.get_omega(...)` therefore always returns the hardcoded AMBER default (180.0°), never a data-driven value, regardless of `(phi,psi,residue)` — treat omega as unsupported by this release rather than as a working lookup. If per-cell uncertainty, spring constants, or the Paper 3 coupling correction are added in a future release, this table will be updated to match.
...
| **MD researcher** (AMBER/tleap) | Not currently supported — see note below | — |
...
| **DNA modeller** | `MechLib_DNA_library.csv` via `DNAGeometryLibrary` | `DNAGeometryLibrary().get(base, bi_bii, pucker_class)` |
...
dna = DNAGeometryLibrary()  # loads MechLib_DNA_library.csv from this directory
geom = dna.get(base='DA', bi_bii='BI', pucker_class='C2-endo')
# Returns a dict of corrected bond lengths/angles for this (base, BI/BII, pucker)
# combination, or None if this exact combination wasn't populated -- unlike the
# protein library, there is no pooled 'ALL' fallback for DNA in this release.
```

> **No native AMBER/tleap parameter file (`.frcmod`) is provided.** Standard
> AMBER `ANGLE`/`BOND` terms are keyed by atom type (e.g. `N -CX-C`), not by
> residue identity, and ff14SB assigns most residues the same generic Cα
> atom type — so this library's per-residue, (φ,ψ)-dependent corrections
> cannot be expressed as a standard frcmod file without custom atom-typing
> per residue, which is out of scope here. AMBER users should apply
> corrections through the OpenMM route above (`amber14-all.xml` loaded into
> OpenMM, then `apply_corrections(...)`), which supports the full
> conformation-dependent library rather than a residue-averaged
> approximation.
...
omega = lib.get_omega(phi=-63, psi=-43, residue='ALA')       # 180.0 -- always the AMBER default; omega is not populated in this release (see schema note above)
...
Extracts, validates, and benchmarks a complete geometry library: 5,303 cells × 21 residue classes × 11 observables, plus 2,987 χ₁-dependent entries. Reduces MAE by 10–36% vs AMBER on all observables. Strain energy analysis: 9.8 kcal/mol phantom strain per 100 residues eliminated. Zero overfitting on temporal split. Integrates with AMBER (−31.5%), CHARMM (−44.8%), and OPLS (−37.4%) via OpenMM. *(These per-observable numbers are from the original library release; the independently re-verified rebuild described in the Update section below reports 38.7% overall AMBER strain reduction using corrected reference parameters — see `mechlib_rebuild/README.md`.)*
...
## Update: Independently Verified Rebuild (2026)

Following institutional relocation, the geometry library was rebuilt on an
independently re-curated dataset with every reference force-field
parameter checked directly against its authoritative primary source
(AMBER ff14SB, OPLS-AA/M, CHARMM36, AMBER parm10/bsc1), rather than
propagated from earlier tabulations. This correction, plus a temporally
blind train/test validation and extension to the DNA sugar-phosphate
backbone, are described in `mechlib_rebuild/README.md` and the updated
manuscript in `manuscript/`. Numbers in that rebuild (e.g. 38.7% vs.
AMBER ff14SB) differ from the original 31.5% figure above due to the
parameter correction, not a change in method — see `mechlib_rebuild/README.md`
