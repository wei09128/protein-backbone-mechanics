#!/usr/bin/env python3
"""Frozen 1 ns pilot scalar coupling analysis; positive paired MSE delta favors MechLib."""
import argparse
import csv
import hashlib
import json
...
def main():
    ...
    for case,(filename,expected) in CASES.items():
        ...
        obs[case]=primary
    if a.preflight:
        print('M2E_PREFLIGHT=PASS; 259 file hashes; 113 mapped non-gly NMR rows; no DCD frames scored')
        return
        ...
        for arm in ARMS:
            for seed in SEEDS:
                ...
                lineage_rows.append({'case':case,'arm':arm,'seed':seed,'n_observations':len(exp),
                                     'n_frames':100,'mse_hz2':mse,'rmse_hz':math.sqrt(mse)})
    ...
    summary={'status':status,'scope':'exploratory_1_ns_pilot_not_Gate_M',
             ...
             'no_universal_validation_claim':True}
    (out/'M2E_SCORE_SUMMARY.json').write_text(json.dumps(summary,indent=2)+'\n')
    print('M2E_SCORED='+status,'equal_protein_mean_delta_mse_hz2='+str(pooled),'output='+str(out))
