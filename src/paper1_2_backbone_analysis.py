...
_GROUP_A = {#'steric_N_3A','steric_N_4A','steric_N_5A',
           ...
           # 'steric_clash_psi_plus','steric_clash_psi_minus',
           'improper_ca','sc_contact_nm1_to_bb','sc_contact_np1_to_bb'}
_GROUP_B = {'tau_phi_lj', 'tau_psi_lj',
           'tau_phi_correct','tau_psi_correct',
           'tau_phi_bb_donor','tau_psi_bb_donor',
           'tau_phi_bb_acc','tau_psi_bb_acc',
           'tau_phi_sc_hb','tau_psi_sc_hb',
...
def predict_omega(records, X, feat_names, y_sincos, out_dir):
    """
    Dedicated ω prediction analysis.
 
    ω is nearly always ~180° (trans peptide bond). Interesting cases:
      - Twisted trans:  160–179° (peptide strain under force)
      - Partial twist:  140–160° (rare, often crystal artefact)
      - Cis peptide:    ~0°      (almost always cis-Pro or Gly-Pro)
 
    Key predictors expected:
      - is_pro_np1          (cis-Pro: ~5–10% cis vs ~0.03% non-Pro)
      - sc_contact_np1_to_bb (close contact → twisted peptide)
      - steric_clash features (packing strain)
      - tau_phi / tau_psi    (if backbone is strained, ω twists too)
      - bfactor_ca           (disordered regions → larger ω deviation)
 
    Outputs:
      - R² for cos_omega (the dominant signal — trans=-1, cis=+1)
      - R² for sin_omega (twist direction)
      - Feature importances coloured by group
      - ω deviation histogram by basin + cis/trans breakdown
      - Fig: omega_analysis.png
    """
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt
    ...
    # ── 1. Basic ω statistics ─────────────────────────────────────────────────
    omegas = np.array([r['omega'] for r in records])
    basins = np.array([r['ss_bin'] for r in records])
    res_names = [r['res_name'] for r in records]
 
    # Classification
    def _omega_class(om):
        # Both 179 and -179 are trans — use distance from ±180
        dist_trans = min(abs(wrap(om - 180.0)), abs(wrap(om + 180.0)))
        dist_cis   = abs(om) if abs(om) < 90 else 180 - abs(om)
        if dist_trans < 30:   return 'trans'
        if abs(om) < 30:      return 'cis'
        return 'twisted'
 
    classes = [_omega_class(o) for o in omegas]
    from collections import Counter
    cc = Counter(classes)
    n = len(records)
    print(f"\n  ω classification (n={n:,}):")
    for cls in ['trans', 'twisted', 'cis']:
        print(f"    {cls:8}: {cc[cls]:>6,}  ({100*cc[cls]/n:.2f}%)")
 
    # Cis residue breakdown
    cis_mask = np.array([c == 'cis' for c in classes])
    if cis_mask.sum() > 0:
        cis_res = Counter([res_names[i] for i in range(n) if cis_mask[i]])
        print(f"    Cis residues: {dict(cis_res.most_common(8))}")
        # Check what follows cis residues (i+1)
        is_pro_np1 = np.array([_f(records[i], 'is_pro_np1') if hasattr(records[i], '__getitem__')
                                else records[i].get('is_pro_np1', 0)
                                for i in range(n)], dtype=float)
 
    twisted_mask = np.array([c == 'twisted' for c in classes])
    print(f"    Twisted: mean ω={np.mean(omegas[twisted_mask]):.1f}°  "
          f"std={np.std(omegas[twisted_mask]):.1f}°"
          if twisted_mask.sum() > 0 else "    Twisted: none")
 
    # REPLACE WITH:
    trans_om = omegas[~cis_mask]
    # Circular mean (essential — omega spans ±180 so linear mean is garbage)
    sin_m = np.mean(np.sin(np.radians(trans_om)))
    cos_m = np.mean(np.cos(np.radians(trans_om)))
    circ_mean_trans = np.degrees(np.arctan2(sin_m, cos_m))
    # Circular std via mean resultant length
    R = np.sqrt(sin_m**2 + cos_m**2)
    circ_std_trans = np.degrees(np.sqrt(-2.0 * np.log(R + 1e-10)))
    print(f"\n  Trans peptide: circular mean ω={circ_mean_trans:.3f}°  "
          f"circular std={circ_std_trans:.3f}°")
    print(f"  (Ideal trans = ±180°; circular std measures peptide plane strain)")
 
    # ── 2. Per-basin ω statistics ─────────────────────────────────────────────
    print(f"\n  ω deviation (from 180°) per basin:")
    print(f"  {'Basin':6}  {'n':>6}  {'⟨|Δω|⟩':>8}  {'σ(Δω)':>8}  "
          f"{'% cis':>7}  {'% twisted':>9}")
    delta_omega = np.array([r['delta_omega'] for r in records])
 
    for basin in sorted(BASIN_NAMES):
        mask = basins == basin
        if mask.sum() < 5:
            continue
        do = delta_omega[mask]
        n_cis  = sum(1 for c in [classes[i] for i in range(n) if mask[i]]
                     if c == 'cis')
        n_tw   = sum(1 for c in [classes[i] for i in range(n) if mask[i]]
                     if c == 'twisted')
        print(f"  {BASIN_NAMES[basin]:6}  {mask.sum():>6}  "
              f"{np.mean(np.abs(do)):>8.3f}  {np.std(do):>8.3f}  "
              f"{100*n_cis/mask.sum():>7.2f}  {100*n_tw/mask.sum():>9.2f}")
 
    # ── 3. Ridge + RF prediction of ω ────────────────────────────────────────
    # Targets: sin_omega and cos_omega
    # cos_omega is the primary signal (trans=-1, cis=+1)
    y_so = y_sincos['sin_omega']
    y_co = y_sincos['cos_omega']
 
    # Feature groups for importance colouring
    GROUP_A = {#'steric_N_3A','steric_N_4A','steric_N_5A',
               #'steric_CA_3A','steric_CA_4A','steric_CA_5A',
               #'steric_C_3A','steric_C_4A','steric_C_5A',
               #'steric_O_3A','steric_O_4A','steric_O_5A',
    ...
    def _feat_color(name):
        if name in GROUP_A: return '#1D9E75'
        if name in GROUP_B: return '#378ADD'
        return '#BA7517'
    ...
    # Ridge on cos and sin separately
    print(f"\n  Ridge R² for ω prediction:")
    ridge_results = {}
    for tname, y_t in [('cos_omega', y_co), ('sin_omega', y_so)]:
        m = Ridge(alpha=1.0)
        cv = cross_val_score(m, X_sc, y_t, cv=5, scoring='r2')
        m.fit(X_sc, y_t)
        r2 = r2_score(y_t, m.predict(X_sc))
        ridge_results[tname] = {'r2': r2, 'cv': cv, 'model': m}
        print(f"    {tname:12}: R²={r2:.4f}  CV={cv.mean():.4f}±{cv.std():.4f}")
 
    # Circular R² for reconstructed ω
    pred_omega = np.degrees(np.arctan2(
        ridge_results['sin_omega']['model'].predict(X_sc),
        ridge_results['cos_omega']['model'].predict(X_sc)))
    actual_omega_deg = np.degrees(np.arctan2(y_so, y_co))
    resid_o = np.array([wrap(p - a) for p, a in zip(pred_omega, actual_omega_deg)])
    mean_om = np.degrees(np.arctan2(np.mean(y_so), np.mean(y_co)))
    var_o = np.var([wrap(a - mean_om) for a in actual_omega_deg])
    circ_r2_omega = 1.0 - np.var(resid_o) / (var_o + 1e-10)
    print(f"    circular ω R²: {circ_r2_omega:.4f}")
    print(f"    ω RMSE: {np.sqrt(np.mean(resid_o**2)):.3f}°")
 
    # RF on cos_omega (more interpretable importances)
    print(f"\n  RF R² for cos_omega (cis/trans signal):")
    rf_cos = RandomForestRegressor(n_estimators=200, max_depth=12,
                                    min_samples_leaf=5, n_jobs=-1, random_state=42)
    rf_sin = RandomForestRegressor(n_estimators=200, max_depth=12,
                                    min_samples_leaf=5, n_jobs=-1, random_state=42)
    cv_cos = cross_val_score(rf_cos, X_sc, y_co, cv=5, scoring='r2', n_jobs=-1)
    cv_sin = cross_val_score(rf_sin, X_sc, y_so, cv=5, scoring='r2', n_jobs=-1)
    rf_cos.fit(X_sc, y_co); rf_sin.fit(X_sc, y_so)
    r2_cos = r2_score(y_co, rf_cos.predict(X_sc))
    r2_sin = r2_score(y_so, rf_sin.predict(X_sc))
    print(f"    cos_omega: R²={r2_cos:.4f}  CV={cv_cos.mean():.4f}±{cv_cos.std():.4f}")
    print(f"    sin_omega: R²={r2_sin:.4f}  CV={cv_sin.mean():.4f}±{cv_sin.std():.4f}")
 
    # Top predictors (importance averaged across sin and cos)
    imp_avg = (rf_cos.feature_importances_ + rf_sin.feature_importances_) / 2
    top_idx = np.argsort(imp_avg)[::-1][:15]
    print(f"\n  Top 15 predictors of ω:")
    print(f"  {'Feature':30}  {'Importance':>10}  {'Group':>7}")
    for i in top_idx:
        fname = feat_names[i] if i < len(feat_names) else f'feat_{i}'
        grp = 'steric' if fname in GROUP_A else ('force' if fname in GROUP_B else 'context')
        print(f"  {fname:30}  {imp_avg[i]:>10.4f}  {grp:>7}")
 
    # ── 4. Plots ──────────────────────────────────────────────────────────────
    fig = plt.figure(figsize=(20, 12))
    gs = fig.add_gridspec(2, 3, hspace=0.35, wspace=0.3)
 
    # Panel A: ω histogram — full dataset with cis/twisted highlighted
    ax_a = fig.add_subplot(gs[0, 0])
    ax_a.hist(omegas, bins=180, range=(-180, 180),
              color='#888780', alpha=0.7, density=True, label='all')
    # Overlay twisted region
    twisted_om = omegas[twisted_mask]
    if len(twisted_om) > 0:
        ax_a.hist(twisted_om, bins=60, range=(-180, 180),
                  color='#BA7517', alpha=0.8, density=True, label='twisted')
    cis_om = omegas[cis_mask]
    if len(cis_om) > 0:
        ax_a.hist(cis_om, bins=30, range=(-180, 180),
                  color='#E24B4A', alpha=0.9, density=True, label='cis')
    ax_a.set_xlabel('ω (°)')
    ax_a.set_ylabel('Density')
    ax_a.set_title('ω distribution\n(log scale shows rare cis peptides)')
    ax_a.set_yscale('log')
    ax_a.axvline(180, color='#1D9E75', lw=1, ls='--', alpha=0.7, label='ideal trans')
    ax_a.axvline(-180, color='#1D9E75', lw=1, ls='--', alpha=0.7)
    ax_a.axvline(0, color='#E24B4A', lw=1, ls='--', alpha=0.7, label='ideal cis')
    ax_a.legend(fontsize=7)
 
    # Panel B: ω deviation (Δω = ω − 180°) by basin — violin-style boxplot
    ax_b = fig.add_subplot(gs[0, 1])
    basin_list = sorted(BASIN_NAMES.keys())
    data_by_basin = [delta_omega[basins == b] for b in basin_list]
    bplot = ax_b.boxplot(data_by_basin,
                          positions=range(len(basin_list)),
                          patch_artist=True,
                          medianprops=dict(color='black', lw=1.5),
                          showfliers=False,
                          widths=0.6)
    for patch, basin in zip(bplot['boxes'], basin_list):
        patch.set_facecolor(BASIN_COLORS[basin])
        patch.set_alpha(0.75)
    ax_b.set_xticks(range(len(basin_list)))
    ax_b.set_xticklabels([BASIN_NAMES[b] for b in basin_list])
    ax_b.axhline(0, color='black', lw=0.8, ls='--')
    ax_b.set_ylabel('Δω (°) from trans')
    ax_b.set_title('ω deviation per Ramachandran basin\n(0 = perfect trans; IQR shown)')
 
    # Panel C: RF feature importance for ω
    ax_c = fig.add_subplot(gs[0, 2])
    top_names = [feat_names[i] if i < len(feat_names) else f'feat_{i}'
                 for i in top_idx]
    top_imp   = [imp_avg[i] for i in top_idx]
    top_colors = [_feat_color(n) for n in top_names]
    ax_c.barh(range(len(top_names)), top_imp, color=top_colors, alpha=0.85)
    ax_c.set_yticks(range(len(top_names)))
    ax_c.set_yticklabels(top_names, fontsize=8)
    ax_c.set_xlabel('RF importance (avg sin + cos)')
    ax_c.set_title('Top 15 predictors of ω\n(green=steric, blue=force, amber=context)')
    ax_c.invert_yaxis()
 
    # Panel D: Actual ω vs predicted (trans only — zoom in on variation)
    ax_d = fig.add_subplot(gs[1, 0])
    trans_mask = ~cis_mask  # plot only trans for readability
    ax_d.scatter(actual_omega_deg[trans_mask],
                 pred_omega[trans_mask],
                 s=0.5, alpha=0.15, color='#378ADD')
    lo = min(actual_omega_deg[trans_mask].min(), pred_omega[trans_mask].min())
    hi = max(actual_omega_deg[trans_mask].max(), pred_omega[trans_mask].max())
    ax_d.plot([lo, hi], [lo, hi], 'r-', lw=1, alpha=0.7, label='perfect')
    ax_d.set_xlabel('Actual ω (°)')
    ax_d.set_ylabel('Predicted ω (°)')
    rmse_trans = float(np.sqrt(np.mean(
        np.array([wrap(p-a) for p,a in
                  zip(pred_omega[trans_mask], actual_omega_deg[trans_mask])])**2)))
    ax_d.set_title(f'Actual vs predicted ω (trans only)\nRMSE={rmse_trans:.2f}°  '
                   f'R²={circ_r2_omega:.3f}')
    ax_d.legend(fontsize=8)
 
    # Panel E: R² comparison — cos_omega Ridge vs RF
    ax_e = fig.add_subplot(gs[1, 1])
    models_bar = ['Ridge\ncos_ω', 'Ridge\nsin_ω', 'RF\ncos_ω', 'RF\nsin_ω']
    r2_bar     = [ridge_results['cos_omega']['r2'],
                  ridge_results['sin_omega']['r2'],
                  r2_cos, r2_sin]
    cv_bar     = [ridge_results['cos_omega']['cv'].mean(),
                  ridge_results['sin_omega']['cv'].mean(),
                  cv_cos.mean(), cv_sin.mean()]
    colors_bar = ['#1D9E75', '#378ADD', '#1D9E75', '#378ADD']
    ax_e.bar(range(4), r2_bar, color=colors_bar, alpha=0.8)
    ax_e.scatter(range(4), cv_bar, color='black', s=50, zorder=5, label='CV R²')
    ax_e.set_xticks(range(4))
    ax_e.set_xticklabels(models_bar, fontsize=9)
    ax_e.set_ylim(0, 1.05)
    ax_e.set_ylabel('R²')
    ax_e.set_title('ω prediction R²\n(bars=train, dots=5-fold CV)')
    ax_e.legend(fontsize=8)
 
    # Panel F: Δω residuals by is_pro_np1 — test cis-Pro specificity
    ax_f = fig.add_subplot(gs[1, 2])
    # Get is_pro_np1 from feat_names
    pro_feat_idx = next((i for i, k in enumerate(feat_names)
                         if k == 'is_pro_np1'), None)
    if pro_feat_idx is not None:
        is_pro = X[:, pro_feat_idx].astype(bool)
        ax_f.hist(delta_omega[~is_pro], bins=60, alpha=0.6,
                  color='#888780', density=True, label='i+1 ≠ Pro')
        if is_pro.sum() > 0:
            ax_f.hist(delta_omega[is_pro], bins=30, alpha=0.8,
                      color='#E24B4A', density=True, label='i+1 = Pro')
        ax_f.set_xlabel('Δω (°) from trans')
        ax_f.set_ylabel('Density')
        ax_f.set_yscale('log')
        ax_f.set_title('ω deviation: pre-Pro vs non-Pro\n(cis-Pro = large Δω)')
        ax_f.legend(fontsize=8)
    else:
        ax_f.text(0.5, 0.5, 'is_pro_np1 not in features',
                  ha='center', va='center', transform=ax_f.transAxes)
 
    p = out / 'omega_analysis.png'
    plt.savefig(p, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"\n  Fig → omega_analysis.png")
 
    return {
        'circ_r2': circ_r2_omega,
        'r2_cos': r2_cos, 'r2_sin': r2_sin,
        'cv_cos': cv_cos.mean(), 'cv_sin': cv_sin.mean(),
        'rmse': float(np.sqrt(np.mean(resid_o**2))),
        'n_cis': int(cis_mask.sum()),
        'n_twisted': int(twisted_mask.sum()),
    }
...
def main():
    ...
    # ── Part 7: ω prediction ──────────────────────────────────────────────────
    print(f"\n{'='*65}")
    print(f"Part 7: ω (peptide bond) prediction ...")
    omega_r2 = predict_omega(records, X_full, feat_names, y_sincos, out_dir)
    ...
    print(f"  ω CIRCULAR R²:     {omega_r2['circ_r2']:.3f}  "
          f"RMSE={omega_r2['rmse']:.2f}°  "
          f"cis={omega_r2['n_cis']:,}  twisted={omega_r2['n_twisted']:,}")
    print(f"{'='*65}\n")
