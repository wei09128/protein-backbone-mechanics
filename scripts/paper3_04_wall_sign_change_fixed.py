...
def local_wall_sign_change(coupling_map, count_map, phi_centers, psi_centers,
                           wall_axis, wall_position, band_width=20, min_cells=2):
    """Local, narrow-band sign-change test -- complements wall_sign_change().

    wall_sign_change() compares the ENTIRE half-plane on each side of the
    wall (e.g. for phi*=-60, "left" = all phi in [-180,-60], "right" = all
    phi in [-60,+180]). That tests whether the wall is a good split point
    for the WHOLE Ramachandran plane's average coupling -- a different,
    global question from "does coupling flip sign locally, right at the
    wall itself" (the actual physical claim being tested, analogous to
    Paper 1's local steric discontinuity).

    This function instead only uses cells within `band_width` degrees of
    the wall on each side (default +/-20 deg), giving a genuine local test.
    """
    if wall_axis == 'phi':
        left_idx = np.where((phi_centers < wall_position) &
                            (phi_centers >= wall_position - band_width))[0]
        right_idx = np.where((phi_centers >= wall_position) &
                             (phi_centers < wall_position + band_width))[0]
        n_rows = len(psi_centers)
        row_centers = psi_centers
    else:
        left_idx = np.where((psi_centers < wall_position) &
                            (psi_centers >= wall_position - band_width))[0]
        right_idx = np.where((psi_centers >= wall_position) &
                             (psi_centers < wall_position + band_width))[0]
        n_rows = len(phi_centers)
        row_centers = phi_centers

    left_vals, right_vals = [], []
    for ri in range(n_rows):
        if wall_axis == 'phi':
            left_cells = [(coupling_map[ri, pi], count_map[ri, pi]) for pi in left_idx
                         if not np.isnan(coupling_map[ri, pi]) and count_map[ri, pi] >= 5]
            right_cells = [(coupling_map[ri, pi], count_map[ri, pi]) for pi in right_idx
                          if not np.isnan(coupling_map[ri, pi]) and count_map[ri, pi] >= 5]
        else:
            left_cells = [(coupling_map[qi, ri], count_map[qi, ri]) for qi in left_idx
                         if not np.isnan(coupling_map[qi, ri]) and count_map[qi, ri] >= 5]
            right_cells = [(coupling_map[qi, ri], count_map[qi, ri]) for qi in right_idx
                          if not np.isnan(coupling_map[qi, ri]) and count_map[qi, ri] >= 5]

        if len(left_cells) >= min_cells and len(right_cells) >= min_cells:
            lw = np.array([c[1] for c in left_cells]); lv = np.array([c[0] for c in left_cells])
            rw = np.array([c[1] for c in right_cells]); rv = np.array([c[0] for c in right_cells])
            left_vals.append(np.average(lv, weights=lw))
            right_vals.append(np.average(rv, weights=rw))
    ...
    left_arr = np.array(left_vals); right_arr = np.array(right_vals)
    diff = right_arr - left_arr
    n_flip = np.sum(np.sign(left_arr) != np.sign(right_arr))
    t_stat, p_val = sp_stats.ttest_rel(left_arr, right_arr)
    d_pool = np.sqrt((np.var(left_arr) + np.var(right_arr)) / 2)
    cohens_d = (np.mean(right_arr) - np.mean(left_arr)) / d_pool if d_pool > 1e-10 else 0.0

    return {
        'wall_axis': wall_axis, 'wall_position': wall_position, 'band_width': band_width,
        'n_slices': len(left_vals), 'mean_left': np.mean(left_arr), 'mean_right': np.mean(right_arr),
        'mean_diff': np.mean(diff), 'n_sign_flip': n_flip, 'frac_sign_flip': n_flip / len(left_vals),
...
def main():
    ...
    for name, (cmap, cnt, phi_c, psi_c, f0) in coupling_maps.items():
        ...
        for axis, pos in [('phi', -60.0), ('psi', -40.0)]:
            ...
            if res is not None:
                tests.append(res)
                print(f"  {name:>20s}  {axis}-wall at {pos:>4.0f}° (GLOBAL halves): "
                      f"\u0394={res['mean_diff']:+.4f}  flip={res['frac_sign_flip']:.0%}  "
                      f"p={res['p_val']:.2e}")
            res_local = local_wall_sign_change(cmap, cnt, phi_c, psi_c, axis, pos, band_width=20)
            if res_local is not None:
                tests_local.append(res_local)
                print(f"  {name:>20s}  {axis}-wall at {pos:>4.0f}° (LOCAL \u00b120\u00b0 band):   "
                      f"\u0394={res_local['mean_diff']:+.4f}  flip={res_local['frac_sign_flip']:.0%}  "
                      f"p={res_local['p_val']:.2e}")
        results_all[name] = tests
        results_all_local[name] = tests_local
    ...
    # ── Local-vs-global comparison (new) ──────────────────────────────────
    local_lines = []
    local_lines.append("=" * 78)
    local_lines.append("LOCAL (\u00b120\u00b0 band) vs GLOBAL (full half-plane) sign-change comparison")
    local_lines.append("=" * 78)
    local_lines.append("")
    local_lines.append("wall_sign_change() compares the ENTIRE half-plane on each side of the")
    local_lines.append("wall position (a global bipartition test). local_wall_sign_change()")
    local_lines.append("only uses cells within +/-20 deg of the wall (a genuine local test of")
    local_lines.append("whether coupling flips sign right at the wall, the more direct analogue")
    local_lines.append("of Paper 1's local steric discontinuity claim).")
    local_lines.append("")
    for name in results_all:
        global_tests = results_all[name]
        local_tests = results_all_local[name]
        local_lines.append(f"[{name}]")
        for g, l in zip(global_tests, local_tests):
            local_lines.append(
                f"  {g['wall_axis']}-wall at {g['wall_position']:>4.0f}\u00b0:  "
                f"GLOBAL \u0394={g['mean_diff']:+.4f} p={g['p_val']:.2e}  |  "
                f"LOCAL(\u00b120\u00b0) \u0394={l['mean_diff']:+.4f} p={l['p_val']:.2e}")
        local_lines.append("")

    local_report_path = os.path.join(args.out, 'local_vs_global_wall_report.txt')
    with open(local_report_path, 'w') as f:
        f.write('\n'.join(local_lines))
    print(f"  Saved {local_report_path}")
    print('\n'.join(local_lines))
