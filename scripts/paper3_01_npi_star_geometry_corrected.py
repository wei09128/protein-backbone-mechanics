...
def parse_pdb_backbone(path):
    ...
    NOTE on insertion codes: this dict is keyed by integer resSeq only, with
    no insertion-code component. If a chain contains e.g. 100, 100A, 100B
    (all sharing resSeq=100), keeping all of them under one (chain, 100) key
    would silently let the last-parsed one overwrite the others -- a real
    residue's coordinates quietly replaced by a different residue's under
    the same label. To avoid that corruption, any residue carrying a
    non-blank insertion code is skipped entirely here rather than merged in.
    This means insertion-coded residues (and their would-be n->pi* pairs)
    are absent from the output rather than silently wrong; downstream,
    resseq+1 lookups that would have needed one of these residues correctly
    fail to find it and return None (see measure_npi_star), rather than
    pairing atoms that are not really sequence-adjacent.
    """
...
def measure_npi_star(residues, chain, resseq_i, resseq_ip1):
    ...
    if 'N' not in rip1:
        return None
    N_ip1 = rip1['N']
    peptide_length = np.linalg.norm(N_ip1 - C_i)
    # Reject chain breaks
    if peptide_length > 1.6 or peptide_length < 1.2:
        return None
    C_ip1 = rip1['C']
    O_ip1 = rip1['O']
    ...
    #
    # IMPORTANT:
    # This is NOT evidence that an n→π* interaction exists.
    # It only indicates that the backbone geometry is increasingly
    # consistent with the stereoelectronic requirements reported in
    # Bartlett et al. and Newberry & Raines.
    # ------------------------------------------------------------------
    ...
    geometry_class = (
        "unlikely" if geometry_score == 0 else
        "possible" if geometry_score == 1 else
        "consistent" if geometry_score == 2 else
        "strongly_consistent" if geometry_score == 3 else
        "ideal_geometry"
    )
    ...
    return {
        ...
        'npi_disp': disp,
        'npi_distance_ok': int(distance_ok),
        'npi_bd_ok': int(bd_ok),
        'npi_donor_ok': int(donor_ok),
        'npi_disp_ok': int(disp_ok),
        'npi_geometry_score': geometry_score,
        'npi_geometry_class': geometry_class,
    }
...
NPI_DONOR_COLS = ['npi_d_OC_donor','npi_theta_BD_donor','npi_d_OO_donor',
    'npi_theta_donor_donor','npi_disp_donor','npi_distance_ok_donor','npi_bd_ok_donor',
    'npi_donor_ok_donor','npi_disp_ok_donor','npi_geometry_score_donor',]

NPI_ACC_COLS = [
    'npi_d_OC_acc', 'npi_theta_BD_acc', 'npi_d_OO_acc',
    'npi_theta_donor_acc', 'npi_disp_acc', 'npi_distance_ok_acc',
    'npi_bd_ok_acc', 'npi_donor_ok_acc', 'npi_disp_ok_acc',
    'npi_geometry_score_acc',
]


def assign_basin(phi, psi):
    """5-way basin assignment, matching the classifier used consistently
    elsewhere in Paper 3 (paper3_stratified_robustness.py etc.).

    FIXED from the original 3-way version: the original lacked a PPII
    category entirely (PPII residues, psi~150, satisfied 'psi >= 50' and
    were silently folded into 'beta'), and its 'other' branch was dead
    code (the first two conditions were exhaustive for phi<0, and phi>=0
    always hit alphaL, so 'other' could never actually trigger).
    PPII is checked before beta since PPII's phi range is a subset of
    beta's broader phi range (same overlapping-condition issue caught
...
def run_analysis(df, out_dir):
    ...
    for label, col in [("Donor d(O···C)", 'npi_d_OC_donor'),
                       ...
                       ("Donor θ_donor", 'npi_theta_donor_donor'),
                       ("Acceptor θ_donor", 'npi_theta_donor_acc'),
                       ("Donor disp", 'npi_disp_donor'),
                       ("Acceptor disp", 'npi_disp_acc'),
                       ("Donor geometry score", 'npi_geometry_score_donor'),
                       ("Acceptor geometry score", 'npi_geometry_score_acc')]:
    ...
    # Geometry-score distributions. Replaces the old binary strong/present
    # fractions (npi_is_strong_*/npi_is_present_* no longer exist — those
    # were from an earlier energy-threshold version of this script).
    # geometry_score ranges 0-4 (see measure_npi_star).
    for label, col in [("Donor", 'npi_geometry_score_donor'),
                       ("Acceptor", 'npi_geometry_score_acc')]:
        if col in df.columns:
            s = df[col].dropna()
            report.append(f"  {label} geometry_score distribution (n={len(s):,}):")
            for score in range(5):
                frac = (s == score).mean()
                report.append(f"    score={score}  {frac*100:5.1f}%")
    ...
    if 'basin' in df.columns:
        for basin in ['alphaR', 'beta', 'PPII', 'alphaL', 'loop']:
            mask = df['basin'] == basin
            if mask.sum() < 10:
                continue
            sub = df.loc[mask]
            d_col = 'npi_d_OC_donor'
            score_col = 'npi_geometry_score_donor'
            if d_col in sub.columns:
                d = sub[d_col].dropna()
                score = sub[score_col].dropna() if score_col in sub.columns else pd.Series(dtype=float)
                score_str = f"⟨score⟩={score.mean():.2f}" if len(score) else "⟨score⟩=n/a"
                report.append(
    ...
    if 'res_name' in df.columns and 'npi_d_OC_donor' in df.columns:
        ...
        for res in sorted(df['res_name'].unique()):
            ...
            score_sub = df.loc[mask, 'npi_geometry_score_donor'].dropna() \
                if 'npi_geometry_score_donor' in df.columns else pd.Series(dtype=float)
            if len(sub) < 10:
                continue
            res_stats.append({
                'res': res, 'n': len(sub),
                'd_mean': sub.mean(), 'd_std': sub.std(),
                'score_mean': score_sub.mean() if len(score_sub) > 0 else np.nan,
            })
        ...
        for r in res_stats:
            report.append(f"  {r['res']:>4s}  {r['n']:>8,}  {r['d_mean']:>10.3f}  "
                         f"{r['d_std']:>6.3f}  {r['score_mean']:>8.2f}")
    ...
    if 'tau_deg' in df.columns and 'res_name' in df.columns and 'basin' in df.columns:
        ...
        for npi_col in ['npi_d_OC_donor', 'npi_theta_BD_donor',
                        'npi_theta_donor_donor', 'npi_disp_donor']:
        ...
        try:
            ...
            if len(sub) > 1000:
                ...
                # ── STRICTER BASELINE: fine (phi,psi) 10-deg grid + res_name ──
                # The 'basin' column above is a coarse 3-5-way classification
                # (no PPII in the original version) that leaves more residual
                # variance on the table than the rest of Paper 3's analyses,
                # which condition on the full (phi,psi) grid. This repeats
                # the test with that stricter, more consistent baseline to
                # check whether n->pi*'s apparent incremental value survives.
                report.append(f"\n  STRICTER TEST: fine (phi,psi) 10-deg grid baseline (not just basin):")
                sub2 = df[['tau_deg', 'phi_deg', 'psi_deg', 'res_name'] +
                          [c for c in NPI_DONOR_COLS if c in df.columns]].dropna()
                if len(sub2) > 1000:
                    if len(sub2) > 50000:
                        sub2 = sub2.sample(50000, random_state=42)
                    y2 = sub2['tau_deg'].values
                    phi_bin2 = (sub2['phi_deg'] // 10 * 10).astype(int).astype(str)
                    psi_bin2 = (sub2['psi_deg'] // 10 * 10).astype(int).astype(str)
                    grid_dummies = pd.get_dummies(phi_bin2 + '_' + psi_bin2, prefix='grid')
                    res_dummies2 = pd.get_dummies(sub2['res_name'], prefix='res')
                    X_base2 = pd.concat([grid_dummies, res_dummies2], axis=1).values
                    npi_features2 = [c for c in NPI_DONOR_COLS if c in sub2.columns]
                    X_npi2 = sub2[npi_features2].values
                    X_full2 = np.hstack([X_base2, X_npi2])

                    gbr2 = GradientBoostingRegressor(
                        n_estimators=100, max_depth=4, random_state=42)
                    cv_base2 = cross_val_score(gbr2, X_base2, y2, cv=5, scoring='r2')
                    cv_full2 = cross_val_score(gbr2, X_full2, y2, cv=5, scoring='r2')

                    report.append(f"    Baseline ((phi,psi) grid + res_name):  {cv_base2.mean():.4f} \u00b1 {cv_base2.std():.4f}")
                    report.append(f"    + n\u2192\u03c0* features:                     {cv_full2.mean():.4f} \u00b1 {cv_full2.std():.4f}")
                    report.append(f"    \u0394R\u00b2 (stricter baseline):               {cv_full2.mean()-cv_base2.mean():+.4f}")
                    report.append(f"    (compare to \u0394R\u00b2={cv_full.mean()-cv_base.mean():+.4f} using the coarse basin baseline above --")
                    report.append(f"     if the stricter number is meaningfully smaller, the coarse-baseline")
                    report.append(f"     result was partly measuring the missing PPII/grid resolution, not")
                    report.append(f"     genuine independent n\u2192\u03c0* information)")
        except ImportError:
    ...
    if 'phi_deg' in df.columns and 'psi_deg' in df.columns and 'npi_d_OC_donor' in df.columns:
        ...
        score_col = 'npi_geometry_score_donor'
        cols_needed = ['phi_deg', 'psi_deg', 'npi_d_OC_donor'] + \
            ([score_col] if score_col in df.columns else [])
        ...
        if score_col in sub.columns:
            agg_kwargs['score_mean'] = (score_col, 'mean')
        grid = sub.groupby(['phi_bin', 'psi_bin']).agg(**agg_kwargs).reset_index()
        ...
        for basin_name, phi_range, psi_range in [
            ...
            if score_col in sub.columns:
                score = sub.loc[mask, score_col]
                report.append(f"  {basin_name:5s}  n={mask.sum():>7,}  "
                             f"⟨d⟩={d.mean():.3f} Å  ⟨score⟩={score.mean():.2f}")
            else:
                report.append(f"  {basin_name:5s}  n={mask.sum():>7,}  ⟨d⟩={d.mean():.3f} Å")
...
def make_plots(df, out_dir):
    ...
    # ── Plot 2: n→π* geometry consistency score on Ramachandran plane ────
    score_col = 'npi_geometry_score_donor'
    if all(c in df.columns for c in ['phi_deg', 'psi_deg', score_col]):
        fig, ax = plt.subplots(figsize=(7, 6))
        sub = df[['phi_deg', 'psi_deg', score_col]].dropna()
        ...
        grid = sub.groupby(['phi_bin', 'psi_bin'])[score_col].mean()
        ...
        im = ax.pcolormesh(phi_c, psi_c, Z, cmap='viridis', vmin=0, vmax=4,
                          shading='auto')
        plt.colorbar(im, ax=ax, label='⟨geometry score⟩ (0–4)')
        ax.set_xlabel('φ [°]')
        ax.set_ylabel('ψ [°]')
        ax.set_title('n→π* geometry consistency score on Ramachandran plane')
        ax.set_xlim(-180, 180)
        ax.set_ylim(-180, 180)
        ax.set_aspect('equal')
...
def main():
    ...
    # Look for a chain column in the source CSV. Without one, per-residue
    # chain identity has to be inferred from the PDB file alone (see
    # _worker_extract), which silently misassigns chain for any resseq
    # value that repeats across chains -- common, since most multi-chain
    # entries renumber each chain starting near 1. If a chain column exists
    # here, use it and skip that guesswork entirely.
    chain_col = next((c for c in ('chain', 'chain_id', 'auth_chain', 'chainID')
                       if c in df_work.columns), None)
    if chain_col is None:
        print("  WARNING: no chain column found in CSV "
              "(looked for: chain, chain_id, auth_chain, chainID). "
              "Chain identity will be guessed from the PDB file, which is "
              "unreliable for any resseq value that repeats across chains "
              "(the common case in multi-chain structures). Consider adding "
              "a chain column to the feature CSV for correctness.")
    
    # Build lightweight lookup: pdb_id -> list of (df_index, resseq[, chain])
    pdb_groups = defaultdict(list)
    cols = ['_pdb_lower', 'res_idx'] + ([chain_col] if chain_col else [])
    for idx, row in df_work[cols].iterrows():
        if chain_col:
            pdb_groups[row['_pdb_lower']].append(
                (idx, int(row['res_idx']), str(row[chain_col]).strip()))
        else:
    ...
    # Worker now only needs pdb_id, list of (idx, resseq[, chain]), and pdb_dir
...
def _worker_extract(pdb_id, idx_resseq_list, pdb_dir):
    ...
    Args:
        ...
        idx_resseq_list: list of (dataframe_index, resseq) tuples, OR
            (dataframe_index, resseq, chain) tuples if a chain column was
            found in the source CSV (see main()).
    ...
    has_chain = len(idx_resseq_list) > 0 and len(idx_resseq_list[0]) == 3
    
    # Fallback resseq -> chain lookup, only used when the CSV didn't supply
    # chain explicitly. WARNING: this keeps only the first chain seen for a
    # given resseq integer, so it silently mis-assigns chain for any resseq
    # value that recurs across chains -- the normal case in multi-chain
    # structures where each chain's numbering restarts near 1. Prefer
    # passing an explicit chain column through main() instead of relying on
    # this.
    resseq_to_chain = {}
    if not has_chain:
    ...
    results = []
    for entry in idx_resseq_list:
        if has_chain:
            df_idx, resseq, chain = entry
        else:
            df_idx, resseq = entry
        ...
        # Donor: this residue's O → next residue's C. Note resseq+1/-1 is
        # arithmetic, not confirmed sequence adjacency -- if the true next
        # residue is missing (chain break) or was dropped for having an
        # insertion code (see parse_pdb_backbone), the atom lookup inside
        # measure_npi_star simply won't find it and returns None. If a
        # residue does exist at resseq+1 but isn't really bonded to this
        # one, the peptide-bond-length sanity check inside measure_npi_star
        # (rejects C-N distances outside 1.2-1.6 A) catches that case too.
